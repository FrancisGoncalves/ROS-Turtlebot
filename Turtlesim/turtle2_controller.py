...
INITIAL_WAYPOINTS_LIST = [LEFT_EYE_INITIAL_WAYPOINT, [4, 10], MOUTH_INITIAL_WAYPOINT, [4.5, 2], [1, 1]]

# Call spawn service
spawn = rospy.ServiceProxy('/spawn', Spawn)

# Define function that spawns a new turtle
def spawn_turtle():
        spawn(5, 5, 1, 'turtle2')
...
class TurtleController:
    ...
    def __init__(self):
        ...
        self.pub_vel = rospy.Publisher('/turtle2/cmd_vel', Twist, queue_size=10)
        # Subscribe to pose topic
        rospy.Subscriber('/turtle2/pose', Pose, self.callback_pose)
        self.set_pen = rospy.ServiceProxy('turtle2/set_pen', SetPen)
        # Create node
        rospy.init_node('turtle2_controller', anonymous=True)
...
if __name__ == '__main__':
    try:      
        # If there is still not a turtle called 'turtle2', spawn a new turtle with this name
        try:
            spawn_turtle()
        ...
        while node_controller.index < (len(INITIAL_WAYPOINTS_LIST) - 1):
            ...
            if node_controller.index == 3:
                node_controller.calculate_errors()
                node_controller.run_mouth_drawing()
                node_controller.sleep()
            # node_controller.set_next_waypoint(INITIAL_WAYPOINTS_LIST)
            print('finished')
