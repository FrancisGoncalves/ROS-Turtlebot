...
class Turtlebot3Controller:
    ...
    def convertions_between_frames(self, p0, p1):
        # Choose points that define the line to follow
        self.p0_w = np.array(p0)
        self.p1_w = np.array(p1)
...
if __name__ == '__main__':
    try:
        ...
        while not rospy.is_shutdown():
            while node.Position[0] < 2:  
                node.convertions_between_frames([0, 1, 0], [2, 1, 0])   
                node.get_distance_to_line()
                node.control_turtlebot3(2, 4)
                node.pub_vel_turtlebot3()
                node.sleep()
            while node.Position[1] < 3:  
                node.convertions_between_frames([2, 1, 0], [2, 3, 0])   
                node.get_distance_to_line()
                node.control_turtlebot3(2, 4)
                node.pub_vel_turtlebot3()
                node.sleep()
            while node.Position[0] > 0:  
                node.convertions_between_frames([2, 3, 0], [0, 3, 0])   
                node.get_distance_to_line()
                node.control_turtlebot3(2, 4)
                node.pub_vel_turtlebot3()
                node.sleep()
            while node.Position[1] > 1:  
                node.convertions_between_frames([0, 3, 0], [0, 1, 0])   
                node.get_distance_to_line()
                node.control_turtlebot3(2, 4)
                node.pub_vel_turtlebot3()
                node.sleep()
