...
CIRCLE_CENTER_w = np.array([1, 1, 0])
CIRCLE_RADIUS = 1
...
class Turtlebot3Controller:
    ...
    def __init__(self) -> None:
        ...
        rospy.Subscriber('/map', Odometry, self.odom_callback)
    ...
    def odom_callback(self, data):
        ...
        self.tf = transform3d.Transform(p=self.Position, quat=Quaternion)
        # Convert quaternion into euler angles
        euler = transforms3d.euler.quat2euler(Quaternion)
        # Get the yaw angle
        self.yaw = euler[0]

    def get_circle_closest_point(self):
        # Get circle closest point relative to the robot in the world frame
        x_closest_w = CIRCLE_CENTER_w[0] + CIRCLE_RADIUS * np.cos(np.arctan2(self.Position[1]-CIRCLE_CENTER_w[1], self.Position[0]-CIRCLE_CENTER_w[0]))
        y_closest_w = CIRCLE_CENTER_w[1] + CIRCLE_RADIUS * np.sin(np.arctan2(self.Position[1]-CIRCLE_CENTER_w[1], self.Position[0]-CIRCLE_CENTER_w[0]))
        # Get tangent vector of that circle point
        x_closest_dot_w = -CIRCLE_RADIUS * np.sin(np.arctan2(self.Position[1]-CIRCLE_CENTER_w[1], self.Position[0]-CIRCLE_CENTER_w[0]))
        y_closest_dot_w = CIRCLE_RADIUS * np.cos(np.arctan2(self.Position[1]-CIRCLE_CENTER_w[1], self.Position[0]-CIRCLE_CENTER_w[0]))
        return [x_closest_w, y_closest_w, 0], [x_closest_dot_w, y_closest_dot_w, 0]
    ...
    def convertions_between_frames(self):
        # Get circle closest point and tangent vector
        circle_closest_point, tangent_vector = self.get_circle_closest_point()
        # print(f'Circle closest point: {circle_closest_point}')
        # print(f'Tangent vector: {tangent_vector}')
        self.p0_w = np.array(circle_closest_point)
        # Define another point along the tangent line to be able to define a line
        self.p1_w = circle_closest_point + 1 * np.array(tangent_vector)
        # Conver the points into world frame
